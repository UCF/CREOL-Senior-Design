...
function sd_orderby_semester_date( $clauses, $query ) {
    ...
    if ( 'sd_semester_date' === $query->get('orderby') ) {
        ...
        if ( ! empty($selected_years) || ! empty($selected_semesters) ) {
            $conditions = array();
            // Mapping of semester names to the numeric suffix stored in semester_date.
            $semesterMap = array(
                'Spring' => '1',
                'Summer' => '2',
                'Fall'   => '3'
            ...
            // If both years and semesters are provided, build a list of allowed exact values.
            if ( ! empty($selected_years) && ! empty($selected_semesters) ) {
                $values = array();
                foreach ( $selected_years as $year ) {
                    foreach ( $selected_semesters as $sem ) {
                        if ( isset( $semesterMap[$sem] ) ) {
                            // Build value in the format "YYYY.X"
                ...
                // If only years are selected, match any term meta starting with the year.
                ...
                // If only semesters are selected, match any term meta ending with the corresponding numeric suffix.
                if ( ! empty($selected_semesters) ) {
                    ...
                    if ( $semConditions ) {
                        $conditions[] = "(" . implode(" OR ", $semConditions) . ")";
                    }
                }
            }
            // Combine any conditions with OR.
            if ( ! empty( $conditions ) ) {
                $clauses['where'] .= " AND (" . implode(" OR ", $conditions) . ") ";
            }
        }
    }
 ...
 * Includes two multi-select dropdowns:
 *   - One for academic years (dynamically generated from term meta values)
 *   - One for semesters (a fixed list: Spring, Summer, Fall)
 *
...
function sd_project_display($atts) {
    ...
    $selected_semesters = isset($_GET['selected_semesters']) ? array_map('sanitize_text_field', (array) $_GET['selected_semesters']) : array();
    $selected_years = isset($_GET['selected_years']) ? array_map('sanitize_text_field', (array) $_GET['selected_years']) : array();
    $search = isset($_GET['search']) ? sanitize_text_field(wp_unslash($_GET['search'])) : '';    
    $paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
    
    // Build a unique cache key (optional caching).
    ...
    $args = array(
        ...
        // Pass our custom query variables for filtering.
        'selected_semesters' => $selected_semesters,
        'selected_years'     => $selected_years,
    );
    // No explicit tax_query is needed since filtering is handled via our posts_clauses filter.
    ...
    // Retrieve all sd_semester terms to build the dynamic year list.
    $terms = get_terms(array(
        'taxonomy'   => 'sd_semester',
        'hide_empty' => false,
    ));
    
    // Build the dynamic list of years from the 'semester_date' term meta.
    $years = array();
    foreach ($terms as $term) {
        // Expected format is "YYYY.X"
    ...
    // Fixed list for the semester dropdown.
    ...
    echo '              </select>';
    echo '          </div>';
    // Multi-select for years (dynamic list).
